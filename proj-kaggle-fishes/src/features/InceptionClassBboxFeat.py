...
trn_bbox_onehot0 = to_categorical(trn_bbox[:,0],nb_classes=Nbins+2)
...
conv_feat = base_model.predict(trn_np)
conv_test_feat = base_model.predict(test_np)
...
x_bb1 = Dense(Nbins+2, activation='softmax', name='bb1')(x)
x_bb2 = Dense(Nbins+2, activation='softmax', name='bb2')(x)
x_bb3 = Dense(Nbins+2, activation='softmax', name='bb3')(x)
x_bb4 = Dense(Nbins+2, activation='softmax', name='bb4')(x)
#predictions = Dense(Nbins+1, activation='softmax', name='bb')(x)

# this is the model we will train
model = Model(input=base_model.input, output=[x_bb1, x_bb2, x_bb3, x_bb4])
...
batch_size=300
earlistop = EarlyStopping(monitor='val_loss', min_delta=0, patience=4, verbose=1, mode='auto')
...
model.compile(Adam(lr=0.001),loss=4*['categorical_crossentropy'],
    metrics=['accuracy'])
