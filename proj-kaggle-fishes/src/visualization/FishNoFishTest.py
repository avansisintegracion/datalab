...
fish_label = pd.read_csv(PROCESSED + 'fish_label.csv', names= column_name)
...
for i,j in enumerate(g):
    bb = fish_label.loc[fish_label['image'] == g[i]].iloc[:,[0,1]].values
    im = cv2.imread(g[i])
    ...
    plt.text(100, 100, bb[0,][1], fontsize=18, color='red')
    name=CROP_TEST_DIR + 'Label' + j
    print(name)
    print(fish_label.loc[fish_label['image'] == g[i]].iloc[:,[0,1]].values)
    plt.savefig(name)
    plt.cla()
    del bb
